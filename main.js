...
module.exports = function (options) {
    //Build the config based on user options and defaults
    var defaultConfig = {
        port: 2947,
    ...
    if (!fs.existsSync(config.configFile)) {
        console.error('The config file "' + config.configFile + '" does not exist.\r\nAborting.')

        return
    }

    //Inject the config from the config file
    var fileConfig = require(path.resolve(config.configFile))
    config = Object.assign({}, config, fileConfig)

    //Get tmp file if supplied
    var backup = {}
    if (config.tmpFile) {
        //Resolve relative tmpfile path and create empty file if not exists
        config.tmpFile = path.resolve(config.tmpFile)
        if (!fs.existsSync(config.tmpFile)) {
            fs.writeFileSync(config.tmpFile, "{}")
        }
        backup = require(config.tmpFile)
    }
    ...
    //Tell user that new client connected
    server.on("connection", function() {
        console.log('New gpsd client connected')
    })

    //Listen to Port
    server.listen(config.port, function () {
        console.log('Fake gpsd server listening at', config.port)
    });
    ...
    setInterval(function() {
        ...
        //Backup data to disk
        if(config.tmpFile) {
            fs.writeFileSync(config.tmpFile, JSON.stringify({
